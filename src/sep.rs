...
use crate::cast_struct;
use crate::filewrite;
use crate::cast_struct_args;
use memchr::memmem;
use std::{
    fs::{File},
    str,
...
pub fn split64(hdr_offset: usize, kernel: &[u8], outdir: &Path, mut outbuf: BufWriter<StdoutLock>, ver: u8) -> Result<(), std::io::Error> {
...
pub fn split32(kernel: &[u8], outdir: &Path, mut sep_info: SEPinfo, mut outbuf: BufWriter<StdoutLock>) -> Result<(), std::io::Error> {
...
pub fn sep32_structs(krnl: &[u8]) -> SEPinfo {
...
pub fn find_off(krnl: &[u8]) -> (u64, u8) {
...
pub fn test_krnl(krnl: &[u8]) -> Option<Vec<u8>> {
    if krnl[..2] == [0x30, 0x83] {
